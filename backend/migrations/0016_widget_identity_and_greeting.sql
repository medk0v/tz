...
WITH normalized AS (
    SELECT
        ...
        (launcher - 'show_operator_message_cta') || jsonb_build_object(
            'support_name',
            CASE
                WHEN jsonb_typeof(launcher->'support_name') = 'string'
                    AND char_length(btrim(launcher->>'support_name')) <= 80
                    THEN to_jsonb(btrim(launcher->>'support_name'))
                ELSE '""'::jsonb
            END,
            'show_greeting',
            CASE
                WHEN jsonb_typeof(launcher->'show_greeting') = 'boolean'
                    THEN launcher->'show_greeting'
                ELSE 'true'::jsonb
            END
        ) AS launcher
    FROM widget_configs
